...
// ─── Taxonomy from EXAM_TAXONOMY.js (imported — no inline duplication) ────────
// EXAM_TAXONOMY structure: { "jee-main": { name, subjects: [{name, chapters: [{name, topics:[{name}]}]}] } }
// Map exam display name → EXAM_TAXONOMY slug
const EXAM_SLUG_MAP = {
  "JEE Main":     "jee-main",
 ...
 * Returns null if exam not found (caller falls back to DB).
 */
function getTaxonomyChapters(examName, subjectName) {
  const slug = EXAM_SLUG_MAP[examName];
  if (!slug || !EXAM_TAXONOMY[slug]) return null;
  ...
  const matched  = subjectName
    ...
    : null;
  const list = matched ? [matched] : subjects;
  const names = list.flatMap(s => (s.chapters || []).map(c => c.name));
  return names.length > 0 ? [...new Set(names)] : null;
}

/**
 * Returns topic name strings for exam+subject+chapter from EXAM_TAXONOMY.
...
function getTaxonomyTopics(examName, subjectName, chapterName) {
  ...
  for (const subj of subjects) {
    if (subjectName && subj.name.toLowerCase() !== subjectName.toLowerCase()) continue;
    const ch = (subj.chapters || []).find(c => c.name.toLowerCase() === chapterName.toLowerCase());
    if (ch) return (ch.topics || []).map(t => t.name);
  }
  return null;
}
